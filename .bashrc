#
# ~/.bashrc
# Credit to Andrew T. for providing this wonderful bashrc
#
...
detailed_stat() { 
    ...
    ctmp=$(sensors | grep "^Core ([0-9]+):" | awk '{print "      " $2 " " $3}')
    if which sensors &>/dev/null && [ -n "$ctmp" ]; then
...
if [[ $_workstations =~ $HOSTNAME ]]; then
	...
	function fsh () {
		ssh -tqi ~/.ssh/id_rsa-fir fir "sudo bash -i -c \"ssh -q $@\""
	}
